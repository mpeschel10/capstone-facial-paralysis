...
export function response200LoginJson(user) {
    const response = Response.json(makeLoginCookie(user), {status: 200});
    console.debug("Respond:", response.status, Object.fromEntries(response.headers));
    return response;
}

export function response303Login(user) {
    const status = 303;
    const Location = user.kind === "ADMIN" ? "/dashboard" : "/home";
    const headers = {Location, "Set-Cookie": makeLoginCookie(user)};
