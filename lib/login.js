...
import {
    response400Custom, response400MissingParameter,
...
export async function requestToUser(request) {
    const [errorResponse, usernamePassword] = await requestToUsernamePassword(request);
    if (errorResponse !== null) return [errorResponse, null];
    ...
    if (!await argon2.verify(expectedHash, password))
        return [response401WrongPassword({ username }), null];
    
    return [null, {user_id: row.id, username: username, kind: row.kind}];
}