...
function saveFile(fieldName, stream, info) { return new Promise((resolve, reject) => {
	const { filename, encoding, mimeType } = info;
	console.log(`kmulter: File [${fieldName}]: filename: %j, encoding: %j, mimeType: %j`, filename, encoding, mimeType);

	const destName = filename;
	const destPath = `public/uploads/${destName}`;
	...
	stream.on('data', (data) => {
		...
		resolve(path.relative('public', destPath));
	});
})}
...
export function saveRequest(request) { return new Promise((resolve, reject) => {
    ...
    bb.on('file', (fieldName, stream, info) => {
        pathPromises.push(
            saveFile(fieldName, stream, info)
        );
    });
